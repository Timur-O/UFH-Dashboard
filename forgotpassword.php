...
<head>
  ...
  <title>
    Forgot Password - Ultifree Hosting
  </title>
          ...
          <div class="col-md-12">
            ...
            <?php
                ...
                foreach($allAnnouncements as $row) {
                    if ($row["location"] == "forgotpassword") {
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    if (empty($_POST["email"])) {
                        ...
                        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            ...
                            $sql = "SELECT * FROM `clients` WHERE `email` = '$emailSQL'";
                ...
                <div class="col-xl-4 col-md-8 offset-xl-1 offset-md-2">
                    ...
                    <div class="card">
                        <div class="card-body ">
                            ...
                            <?php
                             if ($emailErr != "") {
                                 $errorBorder = " border border-danger";
                             } else {
                                 $errorBorder = "";
                             }
                             if (!$resetStart) {
                                 echo '
                                    <p class="text-center">
                                        Enter the email address you used to sign up. We will send you a link to reset your password.
                                    </p>
        
                                    <form action="forgotpassword.php" method="post">
                                        <div class="form-group">
                                            <label for="email">Email address</label>
                                            <input type="email" class="form-control' . $errorBorder . '" id="email" name="email" placeholder="Enter your email..." required>
                                            <small id="emailError" class="form-text text-danger">' . $emailErr . '</small>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Send Me The Reset Link</button>
                                    </form>
                                    <p class="text-center"><a href="login.php">Nevermind, I remembered my password!</a></p>
                                 ';
                             } else {
                                 echo '
                                    <p class="text-center">
...
function sendReset($email, $conn) {
    ...
    $getClientIDSQL = "SELECT `clientID` FROM `clients` WHERE `email` = '$emailSQL'";
    $getClientIDResult = $conn->query($getClientIDSQL)->fetch_assoc();
    $clientID = $getClientIDResult['clientID'];
    $clientIDSQL = $conn->real_escape_string($clientID);

    $resetHash = md5(rand(0,1000));
    $resetHashSQL = $conn->real_escape_string($resetHash);
    $updateResetHashSQL = "UPDATE `clients` SET `passReset` = '$resetHashSQL' WHERE `clientID` = '$clientIDSQL'";
    $updateResetHashResult = $conn->query($updateResetHashSQL);

    if (!$updateResetHashResult) {
        // Database Error
        $errorMessage = $conn->real_escape_string($conn->error);
        $errorLocation = "Forgot Password (DB Error)";
    ...
    // Content
    $mail->isHTML(true);
    $mail->Subject = 'Reset your Ultifree password';
    $mail->Body    = "<!DOCTYPE html PUBLIC '-//W3C//DTD XHTML 1.0 Transitional //EN' 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'>
      ...
      <p style='font-size: 14px; line-height: 1.8; word-break: break-word; mso-line-height-alt: 25px; margin: 0;'><span style='font-size: 14px;'>If you didn't attempt to reset your password, ignore this email.</span></p>
    ...
    $mail->AltBody = 'Hello there,
      ...
      Need to reset your password? No problem! Simply click the link below.
      
      ' . $verificationURL . ' 
      
      If you did not attempt to reset your password, ignore this email.
